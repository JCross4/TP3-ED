		...
		if (strCodP != "" && strNombre != "" && strCodC != "" && strCodR != "") {
			...
			if (APaises->ValidarPais(APaises->raiz, intCodP)) {
				...
				if (!ARest->validarRestaurante(ARest->getRaiz(), intCodR)) {
					...
					MessageBox::Show("Se insertó correctamente", "Restaurante", MessageBoxButtons::OK, MessageBoxIcon::Asterisk);
				}
				else
				{
					MessageBox::Show("Ya existe un restaurante con ese código", "Restaurante", MessageBoxButtons::OK, MessageBoxIcon::Error);
				}
				/*}
				else {
